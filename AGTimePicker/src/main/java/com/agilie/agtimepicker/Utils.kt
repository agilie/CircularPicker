...
fun getPointOnBorderLineOfCircle(center: PointF, radius: Float, alfa: Int = 0) =
        PointF().apply {
            x = (radius * Math.cos(Math.toRadians(alfa - 90.0)) + center.x).toFloat()
            y = (radius * Math.sin(Math.toRadians(alfa - 90.0)) + center.y).toFloat()
        }

fun calculateAngleWithTwoVectors(touch: PointF?, center: PointF?): Int {
    var angle = 0.0
